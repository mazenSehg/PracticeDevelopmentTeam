...
<div id="edit-data-modal-body"></div>
		...
		public function add__booking__page(){
			...
			<form class="add-booking submit-form" method="post" autocomplete="off">
				...
				<div class="form-group">
					...
					<select name="nurses[]" class="form-control select_single require select-nurses" data-placeholder="Choose nurse(s)" multiple="multiple"></select>
				</div>
				<div class="ln_solid"></div>
				<div class="form-group">
			...
			<div id="booking-data-modal" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
				<div class="modal-dialog">
					<div class="modal-content">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
							<h1 class="modal-title text-center text-uppercase"><?php _e('Available Courses');?></h1>
						</div>
						<div class="modal-body">
			...
			<div id="add-booking-modal" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
				<div class="modal-dialog">
					<div class="modal-content">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
							<h1 class="modal-title text-center text-uppercase"><?php _e('Add Course Booking');?></h1>
						</div>
						<div class="modal-body">
		...
		public function show__form__modal(){
			ob_start(); ?>
			<!-- calendar modal -->
			<div id="edit-course-modal" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
				<div class="modal-dialog">
					<div class="modal-content">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
			...
			<div class="hidden edit-course-modal-btn" data-toggle="modal" data-target="#edit-course-modal"></div>
			<?php
			return ob_get_clean();
		}
		...
		public function get__calender__booking__data(){
			...
			if($results):
				...
				foreach($results as $data):
					array_push($return, array('title' => 'Active Courses ('.$data->count.')' , 'start' => date('Y-m-d',strtotime($data->date_from ) ) , 'end' => isset($data->date_to) && $data->date_to != '' ? $this->add_date($data->date_to, '1 day') : $this->add_date($data->date_from, '1 day') ) );
				endforeach;
			...
			<table class="table table-striped table-bordered" cellspacing="0" width="100%">
				...
				<tbody>
					...
					<tr>
						...
						<td><?php echo date('M d,Y',strtotime($booking->date_to));?></td>
						<td class="text-center">
							<?php if( user_can('edit_booking') ): ?>
							<button type="button" class="btn btn-success btn-xs view-nurses" data-toggle="modal" data-target="#nurse-data-modal" data-booking="<?php echo $booking->ID;?>" onclick="get_nurses(this);"><i class="fa fa-view"></i>&nbsp;<?php _e('View Trainee(s)');?></button>
...
//			echo $cID;
		...
		public function nurse__attendance__process(){
			...
			if($booking):
				$attendance = maybe_unserialize($booking->attendance);
				if(!empty($attendance) && isset($attendance[$user_id]) ):
					$attendance[$user_id] = 1;
					$update = $this->database->update(TBL_BOOKINGS, array('attendance' => $attendance), array('ID' => $booking->ID));
					if($update){
						$return['status'] = 1;
						$html = '<label class="label label-info">'.__('Attended').'</label>';
						$enroll = maybe_unserialize($booking->enroll);
						if(!empty($enroll) && isset($enroll[$user_id]) ):
							if($enroll[$user_id] == 1){
								$html .= '<label class="label label-success">'.__('Completed').'</label>';
							}else{
								$html .= '<button type="button" class="btn btn-xs btn-success complete-nurse" data-booking="'.$booking->ID.'" data-user="'.$user_id.'" onclick="nurse_complete(this);">'.__('Complete').'</button>';
							}
						endif;
						$return['html'] = $html;
					}
